...
(async () => {
  ...
  await test('moment identity: duplicates refused per kind, stats merge repeating + one-time', async () => {
    ...
    assert.strictEqual(G.nameConflict({ id: 'x', name: 'Read', emoji: '📚', once: null }, undefined, now), rec, 'a second repeating Read is a duplicate');
    assert.strictEqual(G.nameConflict({ id: 'x', name: 'Read', emoji: '📚', once: today }, undefined, now), oneOff, 'a second one-time Read is a duplicate');
    assert.strictEqual(G.nameConflict({ id: 'x', name: 'Read', emoji: '📖', once: null }, undefined, now), null, 'a different emoji is a different moment');
    assert.strictEqual(G.nameConflict({ ...rec }, undefined, now), null, 'a moment does not conflict with itself');
    const pastOneOff = habit('o2', '20:00', '21:00', { name: 'Call mom', emoji: '📞', days: [], once: T.addDays(today, -3) });
    const pendingOneOff = habit('o3', '20:00', '21:00', { name: 'Water plants', emoji: '🌵', days: [], once: T.addDays(today, 1) });
    S.state.habits = [rec, oneOff, pastOneOff, pendingOneOff];
    assert.strictEqual(G.nameConflict({ id: 'x', name: 'Call mom', emoji: '📞', once: today }, undefined, now), null, 'a one-time moment whose day already passed no longer blocks re-adding it');
    assert.strictEqual(G.nameConflict({ id: 'x', name: 'Water plants', emoji: '🌵', once: T.addDays(today, 1) }, undefined, now), pendingOneOff, 'but a still-pending (today or future) one-time moment keeps blocking a duplicate');
